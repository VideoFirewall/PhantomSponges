...
class YOLOLayer(nn.Module):
    ...
    def forward(self, p, out):
        ...
        if self.training:
            ...
            io = p.sigmoid()
            io_out = io.clone()
            io_out[..., :2] = (io[..., :2] * 2. - 0.5 + self.grid)
            io_out[..., 2:4] = (io[..., 2:4] * 2) ** 2 * self.anchor_wh
            io_out[..., :4] *= self.stride
            #io = p.clone()  # inference output
            #io[..., :2] = torch.sigmoid(io[..., :2]) + self.grid  # xy
            #io[..., 2:4] = torch.exp(io[..., 2:4]) * self.anchor_wh  # wh yolo method
            #io[..., :4] *= self.stride
            #torch.sigmoid_(io[..., 4:])
            return io_out.view(bs, -1, self.no), p  # view [1, 3, 13, 13, 85] as [1, 507, 85]
...
def convert(cfg='cfg/yolov3-spp.cfg', weights='weights/yolov3-spp.weights', saveto='converted.weights'):
    ...
    # from local_yolos import *; convert('cfg/yolov3-spp.cfg', 'weights/yolov3-spp.weights')
